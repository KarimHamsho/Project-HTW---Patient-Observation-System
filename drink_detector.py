...
class DrinkDetector:
    """
    Erkennt Tassen mit YOLO, berechnet den Abstand zum Mund
    ...
    def check_drinking(self, color_frame, frame_for_display, depth_image):
        ...
        for box in boxes:
            ...
            if label == "cup":
                ...
                # Mitte des boxes
                # cup_top = ((x1 + x2) // 2,  (y1 + y2) // 2 )
                # schwer zu betrachten

                # obere Mitte des boxes
                # cup_top = ((x1 + x2) // 2,  y1)
                # das bringt immer fehler beim lesen
                break
        ...
        if cup_detected:
            ...
            if pose_results.pose_landmarks:
                ...
                if 0 <= eye_center[1] < depth_image.shape[0] and 0 <= eye_center[0] < depth_image.shape[1]:
                    ...
                    eye_depth_value = 0  # oder irgendein ungültiger Wert
            ...
            if eye_depth_value != 0 and cup_depth_value != 0:
                ...
                # dist < self.drinking_distance_threshold and
    ...
    def _calculate_3d_distance(self, eye_center, eye_distance, cup_top, cup_distance):
        ...
        # vlt noch abs(), um die 0.000 zu vermeiden
...
class MainApp:
    def __init__(self):
        # RealSense Pipeline erstellen
        self.pipe = rs.pipeline()
        cfg = rs.config()

        # Depth-Stream und Color-Stream aktivieren
        ...
        # DrinkDetector Instanz erstellen
        ...
        try:
            while True:
                ...
                if not depth_frame or not color_frame:
                    print("Keine gültigen Frames empfangen")
                    continue  # ggf. nächsten Durchlauf abwarten

                # Die Depth-Werte an das RGB-Bild ausrichten und synchronisieren
                ...
                # Trink-Detection auf color_image laufen lassen
                count = self.drink_detector.check_drinking(color_image, frame_for_display, depth_image)

                # Bild anzeigen
                # Bild anzeigen
                cv2.putText(frame_for_display, f"Trinkereignisse: {count}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
                cv2.imshow('Drink Detection', frame_for_display)
           ...
           print(f"Ein Fehler ist aufgetreten: {e}")        
            ...
            # Sicherstellen, dass die Pipeline am Ende geschlossen wird
            self.pipe.stop()
            print("Kamera-Überwachung beendet.")
