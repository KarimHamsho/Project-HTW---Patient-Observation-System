...
def main():
    # Instanz unseres Detektors erstellen
    detector = PositionDetector(pose_model_path="yolov8n-pose.pt")

    # Webcam starten (Index 0 bedeutet die erste angeschlossene Kamera)
    cap = cv2.VideoCapture(0)

    sleeping_start_time = None  # Speichert, wann die Augen das erste Mal geschlossen wurden
    sleep_threshold = 5  # ⏳ Zeit in Sekunden, bevor jemand als "schlafend" erkannt wird (hier: 5 Sekunden)
    sleeping = False  # Initialer Schlafstatus

    while True:
        ret, frame = cap.read()
        if not ret:
            break  # Falls kein Frame gelesen werden konnte, Schleife abbrechen

        # Dieses Frame duplizieren, um darauf Keypoints o. Ä. einzeichnen zu können
        frame_for_display = frame.copy()

        # Position und Augenstatus ermitteln
        position, eyes_closed = detector.detect_position_and_eyes(frame, frame_for_display)

        # Neue Logik für Schlaf-Zeitüberprüfung
        if eyes_closed:
            if sleeping_start_time is None:
                sleeping_start_time = time.time()  # Timer starten
            elif time.time() - sleeping_start_time >= sleep_threshold:
                sleeping = True  # Person schläft offiziell
        else:
            sleeping_start_time = None  # Timer zurücksetzen
            sleeping = False  # Augen offen -> nicht schlafend

        # 💤 Text anpassen: "Sleeping" wenn schlafend, sonst "Awake"
        sleeping_text = "Sleeping" if sleeping else "Awake"

        # Position und Schlafstatus ins Frame schreiben
        cv2.putText(
            frame_for_display,
            f"Position: {position}",
            (30, 50),
        ...
        cv2.putText(
            frame_for_display,
            f"Status: {sleeping_text}",
            (30, 100),
            cv2.FONT_HERSHEY_SIMPLEX,
            1.0,
            (0, 0, 255) if sleeping else (0, 255, 0),  #Rot wenn schlafend,  Grün wenn wach
            2
        )

        # Das bearbeitete Frame (mit den eingezeichneten Keypoints etc.) anzeigen
        cv2.imshow("Output", frame_for_display)

        # Abbruchbedingung, z. B. mit ESC (Keycode 27)
...
# Standard-Python: main() nur ausführen, wenn dieses Skript direkt gestartet wird
