...
class PositionDetector:
    ...
    def detect_position_and_eyes(self, frame, frame_for_display):
        ...
        for result in results:
            # Plot only keypoints on the frame_for_display (no bounding boxes or labels)
            frame_for_display[:] = result.plot(labels=False, boxes=False, conf=False)

            # Each 'result.keypoints' contains the pose keypoints data
            for data in result.keypoints:
                # xy_np shape: (17, 3) - representing 17 keypoints with (x, y, confidence)
                xy_np = data.xy.cpu().numpy()[0]
...
def main():
    ...
    while True:
        ...
        position, sleeping = detector.detect_position_and_eyes(frame, frame_for_display)

        # Position und Augenstatus ins Frame schreiben
        cv2.putText(
            frame_for_display,
            f"Position: {position}",
            (30, 50),                  # x,y-Koordinaten für den Text
            cv2.FONT_HERSHEY_SIMPLEX,  # Schriftart
            1.0,                       # Schriftgröße
            (0, 255, 0),               # Schriftfarbe (BGR: Grün)
            2                          # Dicke der Schrift
        )
        cv2.putText(
            frame_for_display,
